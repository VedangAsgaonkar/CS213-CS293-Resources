...
class BST {
 ...
 public:
  ...
  void external(struct Node* nd,TraversalType_e tt);
  void left(struct Node* nd,TraversalType_e tt);
  void below(struct Node* nd,TraversalType_e tt);
  void right(struct Node* nd,TraversalType_e tt);

  // Traverse the tree as per the given traversal type; this function must be recursive
  void traverse(struct Node* nd, TraversalType_e tt);
...
// Optional: (a) insert a million strings (b) delete each of them
